        ...
        <tbody>
          {grupos.map(({ tipo, label }) => {
            ...
            return (
              <>
                <tr key={`header-${tipo}`}>
                  <td colSpan={14} style={s.grupoHeader}>{label}</td>
                </tr>
                {cats.map((cat) => (
                  <tr key={cat.id} style={s.tr}>
                    <td style={s.tdNome}>{cat.nome}</td>
                    <td style={{ ...s.celula, color: "#6c63ff", fontWeight: 600 }}>
                      {cat.valor_previsto ? fmt(cat.valor_previsto) : "—"}
                    </td>
                    {MESES.map((_, i) => {
                      const mes = i + 1;
                      return (
        ...
        <tbody>
          ...
          <tr>
            <td style={{ padding: "6px 8px" }}>
              <input style={s.inputSimples} placeholder="Nova descrição..." value={novo.descricao}
      ...
      <div style={s.modal}>
        ...
        {form.tipo !== "recebimento" && (
          <>
            ...
            <input style={s.input} type="number" value={form.valor_previsto}
              onChange={(e) => setForm({ ...form, valor_previsto: e.target.value })} placeholder="0,00" />
          </>
        )}
        <div style={{ display: "flex", gap: 10, marginTop: 24 }}>
          <button style={s.btnSecundario} onClick={onFechar}>Cancelar</button>
          <button style={s.btnPrimario} onClick={salvar} disabled={salvando || !form.nome}>
...
export default function App() {
  ...
  const {
    ...
    recarregarCategorias, recarregarAnos,
  } = useGastos(anoSelecionado);

  useEffect(() => {
    if (anos.length && !anoSelecionado) {
      const atual = anos.find((a) => a.ano === anoAtual) || anos[0];
        ...
        <div style={s.headerRight}>
          <div style={s.anoSelector}>
            ...
            <input style={s.inputAno} placeholder="+ Ano" value={novoAno}
              ...
              onKeyDown={(e) => e.key === "Enter" && criarAno()}
              maxLength={4} disabled={criandoAno} />
          </div>
        ...
        <main style={s.main}>
          ...
          {aba === "anuais" && (
            <GastosAnuais
              ...
              onSalvo={recarregarGastosAnuais}
            />
          )}
        </main>
      )}
